...
/**
 * animar pantalla de menu
 */
let asteroids = document.getElementsByClassName('asteroid');
let scaleDefaults = [0.4,0.6,0.2,0.3];
let rotateDefaults = [0,0.2,0.5,0.1];

document.body.addEventListener("mousemove", function(evt) {
  let xRatio = Math.min(1, evt.clientX / document.body.offsetWidth );
  let yRatio = Math.min(1, evt.clientY / document.body.offsetHeight);
  
  for (let i = 0; i < asteroids.length; i++) {
        let scaleDef = scaleDefaults[i];
        let rotateeDef = rotateDefaults[i];

        asteroids.item(i).style.transform = `scale(${scaleDef * xRatio}) rotate(${rotateeDef + yRatio}turn)`;
  }
});
